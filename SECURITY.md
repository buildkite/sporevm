...
| `spore-netd` frame stream and ARP parser | guest-originated Ethernet frames via virtio-net | length-prefixed frame bounds and ARP reply handling are unit and fuzz covered; DNS, IPv4, UDP, TCP, and policy parsers must add focused tests or fuzzing when they land |
