...
| `spore run` legacy and SPIO frames | guest vsock stream | bounded host buffers; legacy exit/timing string parser is unit and fuzz covered; SPIO v1 frame headers, stream ids, payload lengths, flags, per-stream offsets, terminal output frames, and resize payloads are unit covered; malformed frames fail the run. TTY mode exposes a guest PTY to the child process and treats merged terminal bytes as untrusted guest output |
