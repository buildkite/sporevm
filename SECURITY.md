...
- **The monitor is still experimental.** Named lifecycle commands require
  `SPOREVM_EXPERIMENTAL_MONITOR=1`. Monitor processes deny child process
  execution through an embedded macOS sandbox profile or Linux seccomp filter,
  covered by `mise run smoke:monitor-jail`. Broader syscall and filesystem
