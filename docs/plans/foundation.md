...
| libspore shared core | Active | The Zig module is published as `libspore`; `src/api.zig` is the product surface, including raw run, managed fresh run setup, run-from-spore, resume with typed events, named lifecycle helpers, and the first network capability/exact-policy contract. Backend, device, storage, daemon, and CLI modules are not re-exported. |
